...
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/Lawrvdjwbj/ss-go/master/ss_go_centos" -O /etc/init.d/ss-go; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/Lawrvdjwbj/ss-go/master/ss_go_debian" -O /etc/init.d/ss-go; then
			echo -e "${Error} Shadowsocks 服务管理脚本下载失败 !"
			rm -rf "${FOLDER}"
			exit 1
		fi
...
Set_cipher(){
 ...
 ${Tip} chacha20 系列加密方式无需额外安装 libsodium，Shadowsocks Go版默认集成 !" && echo
	read -e -p "(默认: 11. aes-256-gcm ):" ss_cipher
	[[ -z "${ss_cipher}" ]] && ss_cipher="11"
	if [[ ${ss_cipher} == "1" ]]; then
